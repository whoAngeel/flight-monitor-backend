...
function AnimatedMarker({ flight, markersRef }) {
  ...
  // 🔹 Guardamos el marcador en la referencia global
  useEffect(() => {
  ...
  // 🔹 Animamos la posición y rotación
  useEffect(() => {
    const controls = [
  ...
  // 🔹 Actualizamos el marcador en tiempo real
  useEffect(() => {
    const updateMarker = () => {
      ...
      // Actualizamos solo la posición del marcador (no dibujamos líneas)
